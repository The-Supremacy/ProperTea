...
echo "  ArgoCD will now sync deploy/environments/$ENV/apps/ and manage:"
  echo "    - cert-manager, argocd (self), gateway, cloudnativepg, keycloak, and their config"
  echo "    - All apps start with automated.enabled=false — sync wave-by-wave (see Step 12 in README)"
echo ""
