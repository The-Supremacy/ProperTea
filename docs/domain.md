...
|---|---|---|
| **Organization** | A Keycloak-managed tenant. Isolation boundary for all data. | Organization Service |
...
| **Company** (Aggregate Root) | A legal business entity (LLC) that owns properties. Has a unique Code per Organization. Multiple per Organization. | Company Service |

### Maintenance & Operations (Actionable Reality)

| Term | Definition | Owned By |
|---|---|---|
| **Work Order** (Aggregate Root) | Primary entity for all maintenance and inspection tasks. | Work Order Service |
...
1. **Onboarding**: User registers via headless flow. Org + Admin created in Keycloak and local state.
