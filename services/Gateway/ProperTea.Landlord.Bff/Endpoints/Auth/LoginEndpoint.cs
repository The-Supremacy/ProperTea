    ...
    public static IEndpointRouteBuilder MapLoginEndpoint(this IEndpointRouteBuilder app)
    {
        app.MapPost("/login", async (
                ...
                HttpContext context) =>
            {
                ...
                await cache.SetStringAsync(sessionId, JsonSerializer.Serialize(userSession),
                    new DistributedCacheEntryOptions
                    {
                        SlidingExpiration = TimeSpan.FromDays(7)
                    });

                context.Response.Cookies.Append(SessionManagementMiddleware.SessionCookieName, sessionId,
                    new CookieOptions
                    {
                        HttpOnly = true,
                        Secure = true,
