...
namespace TheSupremacy.ProperSagas.UnitTests;

public class SagaOrchestratorBaseTests
{
    [Fact]
    public async Task CompensateCompletedAsync_NoStepsWithCompensation_MarkAsCompensated()
    {
        // Arrange
        var saga = new TestSaga([
            new SagaStep { Name = "Execute", HasCompensation = false },
            new SagaStep { Name = "Finalize", HasCompensation = false }
        ]);
        ...
        // Assert
        compensatedSteps.Count.ShouldBe(0);
    ...
    [Fact]
    public async Task ResumeAsync_SagaDontExist_ThrowsException()
    {
        // Arrange
        var mockRepository = new Mock<ISagaRepository>();
        mockRepository
            .Setup(r => r.GetByIdAsync<TestSaga>(It.IsAny<Guid>()))
        ...
        // Act/Assert
        await Should.ThrowAsync<InvalidOperationException>(() => orchestrator.ResumeAsync(Guid.NewGuid()));
    }
    ...
    [Fact]
    public async Task ValidateAsync_NoPreValidationSteps_RunsOnlyPreValidationSteps()
    {
        // Arrange
        var saga = new TestSaga(
            [
                new SagaStep { Name = "Execute", HasCompensation = true, IsPreValidation = false },
                new SagaStep { Name = "Finalize", HasCompensation = false, IsPreValidation = false }
            ]
        );
        ...
        orchestrator.ValidateStepCalled.ShouldBeFalse();
    }

    [Fact]
    public async Task ValidateAsync_PreValidationSuccess_ReturnsValid()
    {
        // Arrange
        var mockRepository = new Mock<ISagaRepository>();
        var mockLogger = new Mock<ILogger<TestOrchestrator>>();
    ...
    [Fact]
    public async Task ValidateAsync_PreValidationFailed_ReturnsInvalid()
    {
        // Arrange
        var mockRepository = new Mock<ISagaRepository>();
        var mockLogger = new Mock<ILogger<TestOrchestratorWithFailValidation>>();
        var orchestrator = new TestOrchestratorWithFailValidation(mockRepository.Object, mockLogger.Object);
        var saga = new TestSaga();
    ...
    public class TestSaga : SagaBase
    {
        ...
        public TestSaga(List<SagaStep> steps)
        {
            Steps = steps;
        }
    }
    ...
    public class TestOrchestratorWithFailValidation : SagaOrchestratorBase<TestSaga>
    {
        public TestOrchestratorWithFailValidation(ISagaRepository sagaRepository, ILogger logger) : base(sagaRepository, logger)
        {
        }
